...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("1", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("1,2", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("(@1", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("@1", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("(@a)", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("(@1;2)", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
        ("(@1:)", pytest.raises(scpi_protocol.InvalidSyntaxException), None),
    ],
)
